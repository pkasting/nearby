...
void WifiMedium::InitCapability() {
  ...
  if (!p_intf_list) {
    ...
    int i;
    for (i = 0; i < (int)p_intf_list->dwNumberOfItems; i++) {
      ...
      if (WlanGetInterfaceCapability(client_handle, &p_intf_info->InterfaceGuid,
        ...
        int j;
        NEARBY_LOGS(INFO) << "Supported PhyMode:";
        for (j = 0; j < p_intf_capability->dwNumberOfSupportedPhys; j++) {
          NEARBY_LOGS(INFO) << p_intf_capability->dot11PhyTypes[j];
...
api::WifiInformation& WifiMedium::GetInformation() {
  ...
  NEARBY_LOGS(INFO) << "GetInformation";
  ...
  for (i = 0; i < (int)p_intf_list->dwNumberOfItems; i++) {
    ...
    if (p_intf_info->isState == wlan_interface_state_connected) {
      ...
      char str_tmp[DOT11_SSID_MAX_LENGTH];
      strncpy(str_tmp,
      ...
      wifi_information_.ssid = absl::StrFormat("%s", str_tmp);
      ...
      NEARBY_LOGS(INFO) << "Frequency: " << *channel
